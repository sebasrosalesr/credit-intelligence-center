...
import type { CreditRecord, SortDirection, SortColumn } from "../types";
...
export interface UseFiltersReturn {
  ...
  // Computed
  filteredData: CreditRecord[];
  sortedData: CreditRecord[];
  isFiltered: boolean;

  // Actions
  setSearch: (search: string) => void;
  setStatusFilter: (statusFilter: string) => void;
  setBulkList: (bulkList: string) => void;
 ...
 * @param getWorkflowState - Function to get workflow state
 * @returns Filter state and operations
 */
export function useFilters(
  data: CreditRecord[],
  getWorkflowState: (record: CreditRecord) => string
): UseFiltersReturn {
  ...
  const bulkSet = useMemo<Set<string>>(() => {
  ...
  const filteredData = useMemo<CreditRecord[]>(() => {
    return data.filter((rec: CreditRecord) => {
      // Status filter
      if (statusFilter !== "All") {
  ...
  // Sort filtered data
  const sortedData = useMemo<CreditRecord[]>(() => {
    const arr = [...filteredData];
    arr.sort((a: CreditRecord, b: CreditRecord) => {
      ...
      switch (sortBy) {
        ...
        case "Invoice":
          ...
          break;
        case "CreditTotal":
          va = parseFloat(String(a["Credit Request Total"] || 0));
          vb = parseFloat(String(b["Credit Request Total"] || 0));
          break;
        default:
          va = "";
          vb = "";
  ...
  const toggleSort = useCallback((columnKey: string) => {
    if (sortBy === columnKey) {
      setSortDir((prev: SortDirection) => prev === "asc" ? "desc" : "asc");
    } else {
      setSortBy(columnKey as SortColumn);
      setSortDir("asc");
    }
  }, [sortBy]);
